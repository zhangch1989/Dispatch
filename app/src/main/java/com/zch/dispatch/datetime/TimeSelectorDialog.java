...
public class TimeSelectorDialog extends Dialog {
    ...
    public void setCurrentDate(String dateString) {
        ...
        Pattern ny = Pattern.compile(pattern4);
        Pattern n = Pattern.compile(pattern5);
        // 现在创建 matcher 对象
        ...
        Matcher m4 = ny.matcher(dateString);
        Matcher m5 = n.matcher(dateString);
        if (m0.find()) {
        ...
        if (m4.find()) {
            ...
            month = Integer.parseInt(m4.group(2)) - 1;
        }
        if (m5.find()) {
    ...
    public String showTime(int isShowtype) {
        switch (isShowtype) {
            ...
            case TimeConfig.YEAR_MONTH_DAY_HOUR:
                selectTime = PowerDateUtils.getDateStr(year, month, day, hour);
//                selectTime = PowerDateUtils.getDateStr(year, month, day, hour,
//                        minute).substring(0,  PowerDateUtils.getDateStr(year, month, day, hour,
//                        minute).length() - 3);
                break;
            ...
            case TimeConfig.YEAR_MONTH:
                selectTime = PowerDateUtils.getMonthStr(year, month);
                break;
            case TimeConfig.YEAR:
                selectTime = PowerDateUtils.getYearStr(year);
                break;
        }
        return selectTime;
    }
...
//        wv_hour.setAdapter(new NumericWheelAdapter(0, hh));
...
//        wv_minute.setAdapter(new NumericWheelAdapter(0, 0));
